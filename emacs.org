...
#+BEGIN_SRC emacs-lisp :tangle no
  (use-package sauron
    :disabled
    :config
    (bind-key "C-c s" 'sauron-toggle-hide-show)
    (bind-key "C-c t" 'sauron-clear)
    (setq sauron-separate-frame nil)
    (add-hook 'sauron-event-added-functions 'sauron-alert-el-adapter)
    (add-hook 'sauron-event-added-functions
              (lambda (origin prio msg &optional props)
                (when (>= prio 4)
                  (sauron-fx-notify "Emacs" msg 10))))
    (sauron-start))
#+END_SRC
...
*** COMMENT Main mu4e configuration
    ...
    (setq mu4e-contexts
          `(,(make-mu4e-context
            ...
            ,(make-mu4e-context
              ...
              :match-func (lambda (msg)
                            (when msg
                              (mu4e-message-contact-field-matches msg :to '("tutor" "pplsge08"))))
