...
Parenface and Rainbow-Delimiters are nice to show matching parentheses.
This is useful not just for Lisp but also for all sorts of nested 
structures, like in =forest= trees.
...
This sets up ~eldoc~ as well as forces the recompilation of init files on save
to speed up Emacs loading.
     ...
     (use-package auctex
                  ...
                  (add-hook 'LaTeX-mode-hook
                            (defun my-LaTeX-mode-hook ()
                              ...
                              ;; If we use the package csquotes (which we
                              ;; should), typing " now does the right thing
     ...
     (use-package markdown-mode
       ...
       (add-hook 'markdown-mode-hook
               (defun my-markdown-mode-hook ()
                 (flyspell-mode)
                 (reftex-mode)
                 (setq reftex-default-bibliography '(private/bibliography-file))
...
#+BEGIN_SRC emacs-lisp :tangle no
  (use-package mu4e
    ...
    (setq mu4e-user-mail-address-list '("pavel.iosad@ed.ac.uk" "piosad@exseed.ed.ac.uk")
          ...
          mu4e-compose-signature (make-random-signature) ;; comment this out
